...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoginErreur('');
    try {
      const reponse = await fetch('https://kincheck-api.onrender.com/token', {
        method: 'POST',
  ...
  const chargerDonneesAdmin = async () => {
    try {
      const resUsers = await fetch('https://kincheck-api.onrender.com/token/api/v1/users/');
      if (resUsers.ok) setListeUsers(await resUsers.json());
      
      const resHist = await fetch('https://kincheck-api.onrender.com/token/api/v1/historique/');
  ...
  const rechercherVehicule = async (e) => {
    ...
    try {
      const reponse = await fetch(`https://kincheck-api.onrender.com/token/api/v1/vehicules/${encodeURIComponent(plaqueClean)}?agent_username=${user.username}`);
  ...
  const analyserPlaqueParPhoto = async (e) => {
    ...
    try {
      const res = await fetch('https://kincheck-api.onrender.com/token/api/v1/ia/analyser-plaque', {
        method: 'POST',
        body: formData
      });
  ...
  const enregistrerVehicule = async (e) => {
    ...
    try {
      const reponse = await fetch('https://kincheck-api.onrender.com/token/api/v1/vehicules/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (photoVehicule) {
        ...
        await fetch(`https://kincheck-api.onrender.com/token/api/v1/vehicules/${vehiculeCree.id}/upload_photo`, {
          method: 'POST',
          body: formData
        });
      }
  ...
  const creerAgent = async (e) => {
    ...
    try {
      const reponse = await fetch('https://kincheck-api.onrender.com/token/api/v1/users/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (fichierPhoto) {
        ...
        await fetch(`https://kincheck-api.onrender.com/token/api/v1/users/${agentCree.id}/upload_photo`, {
          method: 'POST',
          body: formData
        });
      }
  ...
  const supprimerAgent = async (id, username) => {
    ...
    if (window.confirm(`Voulez-vous supprimer l'agent ${username} ?`)) {
      try {
        const res = await fetch(`https://kincheck-api.onrender.com/token/api/v1/users/${id}`, { method: 'DELETE' });
